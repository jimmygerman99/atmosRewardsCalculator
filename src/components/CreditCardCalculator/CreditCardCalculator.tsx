...
function defaultSpend(cardId: string): CardSpend {
    return { cardId, alaskaHawaiianFlights: 0, other: 0, includeAnniversaryBonus: false, includeBoaBonus: false, bonusSpend: {} };
}
...
export default function AtmosCalculator() {
    ...
    function handleCardChange(ids: string[]) {
        ...
        setSpendMap((prev) => {
            const next = { ...prev };
            ids.forEach((id) => {
                if (!next[id]) next[id] = defaultSpend(id);
            });
            return next;
        });
    }
    ...
    const carRentalMilesPerRental =
        elite !== "none"
            ? CAR_RENTAL_MILES_PER_TIER[elite]
            : hasAtmosCard
              ? CAR_RENTAL_MILES_CARD_HOLDER
              : CAR_RENTAL_MILES_PER_TIER.none;
            ...
            <div className="mb-8 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
                <div className="bg-gradient-to-br from-blue-950 to-blue-800 px-8 py-8 text-center relative">
                    ...
                    <p className="text-blue-200 text-sm max-w-md mx-auto">
                        Estimate the Atmos Miles and Status Points you can earn from flights, portal purchases, and credit card
                        spending.
                    </p>
                    <p className="text-blue-400 text-xs mt-2 max-w-sm mx-auto">
                        Unofficial tool — results are estimates only. Verify with <span className="underline">alaskaair.com</span>
                        .
                    </p>
                    <button
